...
# -*- coding: utf-8 -*-
"""
AddressMap.json conflict validator (Windows-safe, no weird escapes)
...
- Reports overlaps and a concise summary per space.

Usage:
  python validate_address_map_win.py AddressMap.json [--max-queue N] [--strict]
...
def is_comment_key(k) -> bool:
    # 주석 키: 언더스코어 시작 또는 __comment 로 끝나는 키를 모두 무시
    return isinstance(k, str) and (k.startswith("_") or k.endswith("__comment"))
...
def main():
    ...
    for s in spaces:
        ...
        span = f" from {smin} to {smax}" if smin is not None else ""
        print(f"- {s}: {len(s_used)} used range(s){span} | {len(s_resv)} reserved range(s)")
